...
# Page configuration
st.set_page_config(
    page_title="HPV Vaccine Cost-Effectiveness Analysis",
    page_icon="💉",
    layout="wide",
...
def create_visualization(results):
    ...
    fig.update_layout(
        title='HPV Vaccine Cost-Effectiveness Analysis',
        xaxis_title='X Axis',
        yaxis_title='Y Axis',
        showlegend=True,
        height=500
...
def main():
    ...
    # Main header
    st.markdown('<h1 class="main-header">💉 HPV Vaccine Cost-Effectiveness Analysis</h1>', unsafe_allow_html=True)
    st.markdown("**Economic Evaluation of Quadrivalent HPV Vaccination for Cervical Cancer Prevention in India**")
    ...
    with st.sidebar:
        ...
        if isinstance(icer_value, str):
            st.metric("ICER", icer_value, "Cost-Saving")
        else:
            st.metric("ICER", f"₹{icer_value:,.0f}/QALY", "Cost-Saving")
